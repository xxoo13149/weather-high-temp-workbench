param(
  ...
  [string]$ZipPath = (Join-Path $PSScriptRoot "kelly-origin-dist-20260424-kelly-stability.zip"),
...
$kellyResponse = Invoke-RestMethod -Uri "http://127.0.0.1:8081/api/weather/kelly?locationId=shenzhen_szx" -TimeoutSec 20
$dashboardResponse = Invoke-RestMethod -Uri "http://127.0.0.1:8081/api/weather/dashboard?locationId=lagos_los" -TimeoutSec 20
...
[PSCustomObject]@{
  ...
  sampleKellyLocationId = $kellyResponse.location.id
  sampleKellyTargetDate = $kellyResponse.targetDate
  sampleKellyWarningCount = @($kellyResponse.warnings).Count
  sampleKellyMarketCount = @($kellyResponse.markets).Count + @($kellyResponse.inactiveMarkets).Count
  sampleDashboardLocationId = "lagos_los"
  sampleDashboardMetarStationId = if ($dashboardResponse.metar -and $dashboardResponse.metar.observation) { $dashboardResponse.metar.observation.stationId } else { $null }
  sampleDashboardTafStationId = if ($dashboardResponse.taf -and $dashboardResponse.taf.forecast) { $dashboardResponse.taf.forecast.stationId } else { $null }
  sampleDashboardSyncState = $dashboardResponse.sync.state
